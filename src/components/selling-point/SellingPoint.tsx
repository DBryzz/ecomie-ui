import { SELLING_POINTS } from '@/src/constants';
import {
    Accordion,
    AccordionContent,
    AccordionItem,
...
import Stat from '@/src/components/selling-point/Stat';
    ...
    return (
        <div className='flex justify-between flex-wrap'>
            <div className='max-w-[690px] w-full flex flex-col'>
                <div className='flex flex-row items-start justify-start text-[30px] sm:text-[40px] md:text-[60px] font-text-lg-semibold lg:flex-col  pb-4'>
                    <b className='relative tracking-[-0.5px] leading-8 sm:leading-10 md:leading-[70px] h-full min-h-[125px]'>
                        Unparalleled Course
                        <span className='text-mediumgreen-100'>{' Distinction'}</span>
                    </b>
                </div>
                <Accordion type='multiple'>
                    {SELLING_POINTS.map(({ id, title, body }) => (
                        <AccordionItem key={id} value={`${id}`}>
                            <AccordionTrigger className='text-mediumgreen-100 text-lg font-bold'>
                                {title}
                            </AccordionTrigger>
